...
function accordion() {
    ...
    for (let index = 0; index < accordionContainer.length; index++) {
        ...
        button.style.top = "50%";
        button.innerHTML = `<svg width="
        20 " height="
        9 " viewBox="
        0 0 20 9 " fill="
        none " xmlns='http://www.w3.org/2000/svg'> 
        <path d = "M1 1L9.75 7.5L18.5 1"
        stroke = "black"
        stroke - width = "2"
        ...
        const uList = e.getElementsByTagName("ul")[0];
        addClass(uList, "transition duration-200");

        function onClickAccordion() {
            if (uList.className.indexOf("h-0") > -1) {
                addClass(button, "rotate-180");
                addClass(uList, "opacity-100");
                removeClass(uList, "h-0 invisible opacity-0");
            } else {
                removeClass(button, "rotate-180");
                removeClass(uList, "opacity-100");
                addClass(uList, "h-0 invisible opacity-0");
            }
        }
